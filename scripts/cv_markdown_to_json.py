...
def parse_gallery(gallery_dir):
    """Parse gallery items from the _gallery directory."""
    gallery = []
    
    if not os.path.exists(gallery_dir):
        return gallery
    
    for gallery_file in sorted(glob.glob(os.path.join(gallery_dir, "*.md"))):
        ...
        if front_matter_match:
            ...
            gallery_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'gallery'),
                "date": front_matter.get('date', ''),
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add gallery
    cv_json["gallery"] = parse_gallery(os.path.join(repo_root, "_gallery"))
